...
function Shift(props) {
        ...
        <div className="flex justify-center items-center h-screen">
            <div className="w-4/5">
                <div className="flex flex-row justify-between">
                    ...
                    <div className="w-1/3 bg-yellow-500 p-4">
                        <div className="flex flex-col items-center border border-gray-200 rounded-lg shadow md:flex-row md:max-w-xl bg-gray-100">
                            <img referrerPolicy="no-referrer" className="object-cover w-full rounded-t-lg h-100 md:h-auto md:w-48 md:rounded-none md:rounded-l-lg" src={shift.mentor.image} alt=""></img>
                            <div className="flex flex-col justify-between p-4 leading-normal">
                                <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900">{shift.mentor.username}</h5>
                    ...
                    <div className="w-1/3 bg-purple-500 p-4">
                        <div className="flex flex-col items-center border border-gray-200 rounded-lg shadow md:flex-row md:max-w-xl bg-gray-100">
                            <img referrerPolicy="no-referrer" className="object-cover w-full rounded-t-lg h-100 md:h-auto md:w-48 md:rounded-none md:rounded-l-lg" src={shift.mentor.image} alt=""></img>
                            <div className="flex flex-col justify-between p-4 leading-normal">
                                <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900">{shift.mentor.username}</h5>
    ...
    async function handleSubmit(e: BaseSyntheticEvent) {
        ...
        switch (result.status) {
            case 200:
                alert("Succesfully signed up for this 1-on-1 session!")
                break;
...
export default function Home({ shifts }) {
    return (
        <>
        { shifts.map((shift) => (
            (shift.student ? <StudentShift key={shift.id} shift={shift} /> : <Shift key={shift.id} shift={shift} /> )
        ))}
        </>
    )
