...
export default function Show({ paramDate }: InferGetStaticPropsType<typeof getServerSideProps>) {  
        ...
        if(status !== 'loading' && session && session.user){
            ...
            const postData = async () => {
                if(!session.user.personal_meeting_url) {
                    toast.error("Please set a meeting link before signing up for a shift!")
                    return
                }
    ...
    function addTime(time: number) {
        ...
        return <span className='text-black font-semibold'>{time}:00 - {time+1}:00</span>
    }
                        ...
                        <div className='flex justify-center items-center h-screen"'>
                            <div className='grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-5'>
                                {paramDate.times.map((time, index) => (
                                    <div key={index} onClick={handleClick} className='bg-[#d5dede] p-5 rounded-md hover:cursor-pointer hover:scale-110 transition ease-in-out '>
                                            {addTime(time)}
                                    </div>
                                ))}
                            </div>
